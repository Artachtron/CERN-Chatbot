...
@with_session
def get_table(table_data, file_id: str, session: Session):
    statement = select(DataTable).where(
        (DataTable.id == table_data.id) & (DataTable.file_id == file_id)
    )
    return session.exec(statement).first()
...
def insert_table(table_data, file_id: str, session: Session):
    if get_table(table_data, file_id, session=session) is None:
...
@with_session
def get_text(text_data, file_id: str, session: Session):
    statement = select(DataText).where(
        (DataText.id == text_data.id) & (DataText.file_id == file_id)
    )
    return session.exec(statement).first()
...
def insert_text(text_data, file_id: str, session: Session):
    if get_text(text_data, file_id, session=session) is None:
...
@with_session
def get_image(image_data, file_id: str, session: Session):
    statement = select(DataImage).where(
        (DataImage.id == image_data.name) & (DataImage.file_id == file_id)
    )
    return session.exec(statement).first()
...
def insert_image(image_data, file_id: str, session: Session):
    if get_image(image_data, file_id, session=session) is None:
        image = DataImage(
